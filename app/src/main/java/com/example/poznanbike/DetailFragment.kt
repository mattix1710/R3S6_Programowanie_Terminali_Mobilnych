    ...
    fun saveToDatabase() {
        ...
        GlobalScope.launch {
            withContext(Dispatchers.IO) {
                // Check whether a bike station with a given label already exist in the database
                val byLabel = bikeStationDao.getByLabel(args.bikeStation.properties.label)
                val op = if (byLabel.isEmpty()) {
                    // If the bike station isn't in the database insert it and set the message
                ...
                // Display the status of the orientation to the user with a snackbar
                Snackbar.make(
                    binding.root,
